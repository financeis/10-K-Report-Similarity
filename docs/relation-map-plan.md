...
> **상태: 검토용 초안 v2 (2026-09-24). 아직 구현 전입니다.**
...
- **역할 분담:** 저장소 주인은 금융 실무자입니다. 관계 유형의 정의와 정답셋은 저장소 주인이 직접 만듭니다.
- 남은 질문 목록은 15장에 있습니다.
...
3. 관계 판정이 얼마나 맞는지 저장소 주인의 정답셋으로 잽니다.
...
4. **평가 우선.** 판정 모델과 임계값은 정답셋으로 정합니다. 질문을 다듬는 데 쓴 데이터로 최종 성능을 재지 않습니다.
...
- **확인이 필요한 점:** 개발사 자체 측정에서도 최상위 모델보다 약 6%p 낮았습니다. 이 과제에 쓸 만한지는 정답셋 비교로 정합니다(7.6).
...
data/sections (원문, 기존) + documents.parquet (정제 텍스트, 기존) + 유사도 행렬 (기존, tfidf-openai)
   ...
   └─ ⑥ eval-relations 정답셋 채점(개발셋·평가셋 분리), 모델 비교, 관계 유형별 주가 동조성
                             │
                        웹앱 (tenksim serve)
```
...
**규모 [확인]:** S&P 500 476개사에서 언급 쌍을 포함하면 다음과 같습니다. 처음에는 K=20으로 시작하고, 정답셋이 생기면 K=10·20·30의 후보 재현율과 추가 판정 비용을 비교해 정합니다.
...
**임계값.** 질문 유형별로 개발셋(7.6)에서 정합니다. 시작값의 예는 다음과 같습니다.
- 채택: 점수 0.8 이상
- 기각: 0.3 미만
- 그 사이: `uncertain`
...
**데이터 분리.** 질문을 다듬고 임계값을 정한 데이터로 최종 성능을 재면 결과가 좋게 나옵니다. 혼자 라벨링하는 현실을 고려해 두 세트로 시작합니다.

| 세트 | 용도 | 규모 (초안) |
|---|---|---|
| 개발셋 | 질문 문구 수정, 판정 입력 구성 실험, 임계값 결정 | 시범 약 15~20개사 |
| 평가셋 | 최종 성능, 모델 비교. 질문·임계값을 확정한 뒤에만 열어 봄 | 별도 약 15~20개사 |

- 두 세트는 **회사 단위로** 나눕니다. 같은 회사의 쌍이나 같은 10-K 문서가 두 세트에 함께 들어가지 않게 합니다.
- 섹터별로 층화해서 고릅니다.
- 라벨링 여유가 생기면 개발셋에서 임계값 조정용을 따로 떼어 세 세트로 늘립니다.

**정답의 두 범위.** 정답 관계마다 어느 범위에 속하는지 표시합니다.
- **알려진 관계:** 저장소 주인이 아는 실제 관계 전부입니다.
- **확인 가능한 관계:** 해당 연도 Item 1·1A에서 실제로 확인할 수 있는 관계입니다.

파이프라인의 추출 성능은 '확인 가능한 관계'로 잽니다. '알려진 관계'와의 차이는 공시 범위의 한계로 따로 보고합니다(예: Item 7이나 주석에만 있는 고객 공시).
...
**표본과 불확실성**
- 정밀도는 검수로 잽니다. 관계를 유형과 점수 구간으로 층화해 뽑고, 전체 정밀도를 계산할 때는 층별 추출 확률로 가중합니다.
...
**모델 비교.** 같은 입력으로 Jev와 텍스트 LLM 1~2개를 돌려 평가셋에서 비교합니다.
...
5. **판정 전 후보 보기 (0단계):** 판정 모델을 붙이기 전에도 후보와 언급 원문을 확인할 수 있게 합니다. 정답셋을 만들 때도 이 화면을 씁니다.
...
src/tenksim/
  relations/
    ...
    evaluate.py     개발셋·평가셋 채점, 단계별 재현율, 모델 비교, 유형별 주가 동조성
...
relations:
  ...
  datasets: configs/relations_gold.yaml   # 개발셋·평가셋 회사 목록
```
...
스키마·저장 구조를 먼저 확정합니다. 다만 정답셋이 완성될 때까지 화면을 미루지는 않습니다. 후보와 원문을 확인하는 화면이 있어야 정답셋을 만들기도 쉽기 때문입니다.

| 단계 | 할 일 | 끝났다는 기준 | 모델 비용 |
|---|---|---|---|
| **0. 스키마 확정과 뼈대** | 관계·근거·판정 상태·검수 키 확정(이 문서 5·7.5장). mentions·candidates 정식화와 테스트. FastAPI와 '판정 전 후보 보기'·근거 확인 화면 | NVIDIA를 열면 후보와 언급 원문이 위치·출처와 함께 보이고, 원문 추적이 맞음 | 없음 |
| **0과 병행: 정답셋 작성 시작** | 저장소 주인이 개발셋·평가셋 회사를 고르고, 모델 결과를 보기 전에 관계를 적음. 각 관계에 '알려진 / 확인 가능' 범위 표시 | 개발셋 라벨 완료 | 없음 |
| **1. 판정과 평가** | Jev·텍스트 LLM 어댑터, 질문 v2, 캐시와 `eval-relations`를 함께 구현. 개발셋으로 질문·임계값을 정하고 평가셋으로 확인 | 평가셋 기준 단계별 재현율과 유형별 정밀도, 기본 판정 모델 결정 | Jev 몇 달러, 텍스트 LLM 비교 수십 달러 이하 |
...
| 리스크 | 대응 |
|---|---|
| Jev가 이 과제에서 부정확함 (자체 측정에서도 최상위 모델보다 약 6%p 낮음) | 평가셋으로 비교한 뒤 결정. 기각 쪽 감사 표본으로 거짓 음성 확인. 어댑터로 교체 가능 |
| Jev 점수가 이 과제에서 보정되지 않음 | 점수 보정을 확인하기 전까지 확률로 표시하지 않음. 임계값은 개발셋으로 정함 |
...
| 평가가 낙관적으로 나옴 | 개발셋·평가셋을 회사 단위로 분리, 회사 단위 부트스트랩 |
...
4. **정답셋 규모:** 개발셋·평가셋 각 15~20개사로 유형별 판단이 가능할까요? 유형별 최소 표본 수를 어떻게 잡는 게 좋을까요?
