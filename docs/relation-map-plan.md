...
- **남은 알려진 문제:** 줄바꿈이 문장 중간에 들어간 10-K에서는 근거 문장이 문장 중간부터 잘려 보입니다(예: Henry Schein–McKesson "sales, are McKesson Corporation"). `mentions.py`의 문장 나누기를 고쳐야 하는데, 그러면 근거 구간 ID가 바뀌어 dev1·conf1 표본과 판정 캐시에 영향이 가므로 따로 처리합니다.
