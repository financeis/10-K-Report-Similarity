...
> **상태: v2.2 (2026-09-25). 0단계 완료, 1단계(판정과 평가) 진행 중입니다.**
...
| | 할 일 | 상태 |
|---|---|---|
| 2-1 | 전체 판정과 관계 만들기. 제외되지 않은 이름 언급 3,580단위를 모두 Jev로 판정했습니다(`tenksim judge --all`, 404만 토큰, 약 $0.17). 결과를 `graph.db`(스키마 v2: `unit_judgements`, `edges`, `edge_evidence`)에 합쳤습니다. `tenksim export`는 캐시에 있는 판정만 합치고 Jev를 부르지 않습니다. 신용평가·ESG 평가 문맥 제외를 넓혔습니다(7.1). 결과: 관계 1,244개(채택 1,095: 경쟁 563, 공급·협력 513, 지분 19 / 불확실 149), 채택 관계가 하나 이상인 회사 394곳. 지분은 `validated=0`('검증 전') | 완료 (2026-09-25) |
| 2-2 | 화면: 회사 페이지에 관계(유형·근거 문장·검증 전 표시·불확실), 관계도 그림(8.2), 불확실 관계 검수 대기열 | 다음 |
| 2-3 | 관계 유형별 주가 동조성 분석과 리포트(`reports/relations_sp500_2024.md`) | |

## 11. 규모와 비용 추정

가정: 요청 하나에 쌍 하나를 넣고, 요청당 입력은 약 3,000 토큰입니다(근거 구간 + 질문).

**실측 [확인] (1-1, dev1 179단위):** 요청 하나에 검수 단위 하나를 넣는 방식(7.3)에서 단위당 입력은 약 1,300토큰입니다. S&P 500의 검수 단위 3,617개를 모두 판정하면 약 470만 토큰, Jev로 약 $0.20입니다. 동시 6요청으로 초당 약 20건을 처리했습니다.
