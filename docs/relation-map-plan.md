...
- 시점(`status`)은 끝난 관계를 걸러내려고 판정 결과에 남기지만, 조정 대상에서는 뺍니다.
- 검수 화면과 `reviews.sqlite`의 라벨은 아래 표처럼 더 잘게 적혀 있어도 됩니다. 채점할 때 위 세 가지로 묶습니다(`relations/evaluate.py`의 `LABEL_GROUPS`).
...
| 1-2 | 개발 표본으로 질문 문구와 질문별 임계값 조정. 판정 유형을 셋으로 줄인 질문 v3.0과 입력 방식 비교(`--context span/nearby/pair`, 7.3)까지 함: `nearby` 채택, 쌍 단위 정밀도 100%·재현율 91%. 남은 일: 질문별 임계값, 회사 식별 문구(이름이 같은 별도 법인: 보틀러, 합작법인) | 진행 중 |
| 1-3 | 텍스트 LLM 어댑터와 같은 입력으로 비교 | |
| 1-4 | 질문·임계값 고정 → 확인 표본을 새로 뽑아 검수 → 합격 판단(7.6) | |
