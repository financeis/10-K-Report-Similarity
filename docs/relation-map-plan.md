...
3-1까지 만든 구성입니다. 처음 제안과 달라진 점은 아래 '제안과 다른 점'에 있습니다.
...
| 3-2 | 최신 제출 연도(2025) S&P 500 실행, 2024↔2025 관계 비교(새로 생김·끝남), 유사도의 연도 안정성. 시작할 때 아래 (a)·(b)를 고침(새 연도는 어차피 새로 판정하므로) | |
...
**남은 알려진 문제와 할 일 (2026-09-25 점검)**
- **(a) 줄바꿈 때문에 근거 문장이 중간에서 잘림**(8.2 끝): 근거 구간 3,106개 중 41개. 채택 관계 1,095개 중 21개가 잘린 문장을 근거로 갖고, 7개는 잘린 문장에만 기댑니다.
  - 고치면 구간 ID가 바뀝니다. dev1 1단위, conf1 2단위의 라벨을 새 ID로 옮겨야 하고, 재판정은 약 $0.002입니다.
  - 3-2를 시작할 때 (b)와 함께 고칩니다.
- **(b) 목록 도입문이 목록 뒤 일반 문장에도 붙음**: 도입문이 붙은 구간 170개 중 38개는 목록 항목이 아닙니다.
  - 그중 약 21개는 목록이 끝난 뒤의 일반 문장에 엉뚱한 도입문이 붙은 것입니다. 나머지는 풀린 표의 행이나 각주라서 도입문이 쓸모 있습니다.
  - 잘못된 도입문 때문에 틀린 채택 관계는 찾지 못했습니다.
  - 고쳐도 구간 ID는 그대로지만 원문 해시가 바뀝니다(dev1 1단위, conf1 5단위).
