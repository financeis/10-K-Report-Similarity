...
def test_labels_are_validated_and_latest_wins(dbs):
    ...
    with pytest.raises(reviews.LabelError, match="시점"):
        reviews.record_label(rev, graph, is_entity="yes", relations=["competitor"], **kw)
    ...
    with pytest.raises(reviews.LabelError, match="모르는 관계"):
        reviews.record_label(
            rev, graph, is_entity="yes", relations=["customer"], status="current", **kw
        )

    reviews.record_label(rev, graph, is_entity="yes", relations=[], **kw)  # 관계 아님
    reviews.record_label(
        rev, graph, is_entity="yes", relations=["partner", "competitor"], status="current",
        partner_type="licensing", **kw,
    )  # fmt: skip
    label = reviews.latest_label(rev, unit)
    assert label["relations"] == ["competitor", "partner"]  # 정해진 순서로 저장
    assert label["partner_type"] == "licensing" and label["blind"] == 1
